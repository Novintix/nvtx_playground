...
from tools import list_github_repos, get_github_commits, get_task_logs, list_repo_files, get_file_content, save_daily_standup
...
# Define Tools
tools = [list_github_repos, get_github_commits, get_task_logs, list_repo_files, get_file_content, save_daily_standup]
...
Workflow:
3. **Check for Daily Standup**: If the user hasn't provided their goals for the day, ASK: "What are your goals for today?".
4. Once the user answers, use `save_daily_standup` to store it.
5. Identify the repository the user is interested in.
6. If the user provides a username but no repo, use `list_github_repos` to see available repos.
7. **STOP and Ask** the user which repository to check ("Which one would you like the EOD for?").
8. Once the user selects a repo, use `get_github_commits` to fetch all commits.
9. Generate the EOD update based SOLELY on the fetched commits and the Daily Standup comparison.
...
- Adopt the persona of a 10-year Senior Developer. 
- Look for code quality, security, performance, and best practices. 
- Be critical but constructive.
- Use `list_repo_files` to see what files are in the repository before reviewing.
- Use `get_file_content` to read the code.
""")
...
def eod_generator_node(state: AgentState):
    ...
    messages = state["messages"]
    # Extract Daily Standup from history
    daily_standup_plan = "No specific standup goals recorded."
    for msg in messages:
        if hasattr(msg, 'tool_calls'):
            for tc in msg.tool_calls:
                if tc['name'] == 'save_daily_standup':
                    daily_standup_plan = tc['args'].get('plan', '')

    prompt = HumanMessage(content=f"""Based on the gathered GitHub commits and the user's initial Daily Standup, generate a DETAILED and structured End-of-Day (EOD) update.

    User's Daily Standup Goals: "{daily_standup_plan}"
    
    CRITICAL: You must use the FULL commit messages (including descriptions) to provide deep technical insight. Do not just list titles.
    
Format:
## EOD Update for [Repository Name]

### ✅ Plan vs Reality
[Compare the "Daily Standup Goals" above with the actual work found in the commits. Did they achieve their goals? Be specific.]

### 🚀 Key Activity
[Summary of the main work done based on commit messages]

### 📝 Commits
[List of commits with Author, Date, and a brief description of the impact]

(Do NOT include any generic "Simulated" sections or "Task Logs" if they were not fetched.)
(Ensure the "Key Activity" section is substantial and explains WHAT changed and WHY, based on the commit logs.)
""")
