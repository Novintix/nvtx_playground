...
class CommitAnalyzer:
    ...
    def get_deleted_code_between_commits(self, old_commit, new_commit):
        """
        Get code that was deleted between two commits.
        Returns dict with file paths and deleted lines.
        """
        try:
            cmd = [
                "git", "-C", self.repo_path, "diff",
                old_commit, new_commit,
                "--unified=0"  # No context lines, just changes
            ]
            result = subprocess.check_output(cmd, text=True, encoding='utf-8', errors='ignore')
            ...
            for line in result.split('\n'):
                if line.startswith('--- a/'):
                    ...
                    if current_file:
                        ...
                        deletions[current_file].append(line[1:])  # Remove the '-' prefix
    ...
    def analyze_feature_loss(self, initial_commit_index=0, recent_commit_index=-1):
        ...
        deletions = self.get_deleted_code_between_commits(initial_commit, recent_commit)
    ...
    def get_feature_loss_context(self, max_commits=20):
        ...
        for i in range(len(commits) - 1):
            ...
            deletions = self.get_deleted_code_between_commits(older["hash"], newer["hash"])
