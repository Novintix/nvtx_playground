from pathlib import Path
from typing import List
from pypdf import PdfReader
import hashlib
import shutil
from datetime import datetime


from mcp.server.fastmcp import FastMCP
CURRENT_ROOT = None

mcp = FastMCP("local-agent-server")

# ✅ Allowed roots (server-side safety boundary)
ALLOWED_ROOTS = [
    Path.home() / "Music",
    Path.home() / "Documents",
]


def is_allowed_path(p: Path) -> bool:
    p = p.resolve()
    return any(str(p).startswith(str(root.resolve())) for root in ALLOWED_ROOTS)

# This tool allows the client to set a root folder for file operations - restricting access to allowed paths only.
@mcp.tool()
def set_root(folder: str) -> str:
    """
    Client sets a root folder for the current session.
    Server will restrict file ops to this root.
    """
    global CURRENT_ROOT

    folder_path = Path(folder).expanduser().resolve()

    if not folder_path.exists():
        return f"❌ Root folder not found: {folder_path}"

    if not is_allowed_path(folder_path):
        return f"❌ Root not allowed: {folder_path}"

    CURRENT_ROOT = str(folder_path)
    return f"✅ Root set to: {CURRENT_ROOT}"


#this tool lists files in a given folder, with optional filtering by file extensions
@mcp.tool()
def list_files(folder: str, extensions: List[str] = None) -> List[str]:
    """
    List files in a folder (optionally filter by extension).
    """
    folder_path = Path(folder).expanduser()

    if not folder_path.exists():
        return [f"❌ Folder not found: {folder_path}"]

    if not is_allowed_path(folder_path):
        return [f"❌ Access denied. Folder outside allowed roots: {folder_path}"]

    results = []
    for item in folder_path.iterdir():
        if item.is_file():
            if extensions:
                if item.suffix.lower() in [e.lower() for e in extensions]:
                    results.append(str(item))
            else:
                results.append(str(item))

    return results

#this tool searches for files in a given folder matching a query in their names
@mcp.tool()
def search_files(folder: str, query: str) -> List[str]:
    """
    Search for files containing query in name inside a folder.
    """
    global CURRENT_ROOT
    if CURRENT_ROOT:
        folder = CURRENT_ROOT
    folder_path = Path(folder).expanduser()
    

    if not folder_path.exists():
        return [f"❌ Folder not found: {folder_path}"]

    if not is_allowed_path(folder_path):
        return [f"❌ Access denied. Folder outside allowed roots: {folder_path}"]

    query_lower = query.lower()
    matches = []

    for item in folder_path.rglob("*"):
        if item.is_file() and query_lower in item.name.lower():
            matches.append(str(item))

    return matches[:50]

# This tool reads text from a PDF file safely
@mcp.tool()
def read_pdf_text(path: str) -> str:
    """
    Extract text from a PDF file safely.
    """
    global CURRENT_ROOT
    if CURRENT_ROOT:
        folder = CURRENT_ROOT
    file_path = Path(path).expanduser()

    if not file_path.exists():
        return f"❌ File not found: {file_path}"

    if not is_allowed_path(file_path):
        return f"❌ Access denied. File outside allowed roots: {file_path}"

    if file_path.suffix.lower() != ".pdf":
        return f"❌ Not a PDF file: {file_path.suffix}"

    try:
        reader = PdfReader(str(file_path))
        text = ""
        for page in reader.pages:
            page_text = page.extract_text() or ""
            text += page_text + "\n"
        return text.strip()
    except Exception as e:
        return f"❌ Failed to read PDF: {e}"
    
# This tool reads text-like files (txt, md, json, csv) safely
@mcp.tool()
def read_text_file(path: str) -> str:
    """
    Read a text-like file safely (txt, md, json, csv).
    """
    global CURRENT_ROOT
    if CURRENT_ROOT:
        folder = CURRENT_ROOT
    file_path = Path(path).expanduser()

    if not file_path.exists():
        return f"❌ File not found: {file_path}"

    if not is_allowed_path(file_path):
        return f"❌ Access denied. File outside allowed roots: {file_path}"

    allowed_ext = {".txt", ".md", ".json", ".csv"}
    if file_path.suffix.lower() not in allowed_ext:
        return f"❌ Not a readable text file type: {file_path.suffix}"

    return file_path.read_text(encoding="utf-8", errors="ignore")

# ✅ Tool: calculate file hash (for duplicate handling)
@mcp.tool()
def get_file_hash(path: str) -> str:
    """
    Returns SHA256 hash of a file (to detect duplicates even with same name).
    """
    file_path = Path(path).expanduser()

    if not file_path.exists():
        return f"❌ File not found: {file_path}"

    if not is_allowed_path(file_path):
        return f"❌ Access denied: {file_path}"

    try:
        sha256 = hashlib.sha256()
        with open(file_path, "rb") as f:
            for chunk in iter(lambda: f.read(1024 * 1024), b""):
                sha256.update(chunk)
        return sha256.hexdigest()
    except Exception as e:
        return f"❌ Failed to hash file: {e}"


# ✅ Tool: scan folder for resume PDFs
@mcp.tool()
def scan_resumes(folder: str) -> list[str]:
    """
    Scan a folder recursively and return all PDF files.
    """
    folder_path = Path(folder).expanduser()

    if not folder_path.exists():
        return [f"❌ Folder not found: {folder_path}"]

    if not is_allowed_path(folder_path):
        return [f"❌ Access denied. Folder outside allowed roots: {folder_path}"]

    pdfs = []
    for f in folder_path.rglob("*.pdf"):
        if f.is_file():
            pdfs.append(str(f))

    return pdfs[:200]  # limit to avoid huge output


# ✅ Tool: mock upload (copy file to uploads folder)
@mcp.tool()
def upload_to_hrms_mock(source_path: str, dest_folder: str) -> str:
    """
    Mock upload to HRMS:
    Copies the file into dest_folder (like uploads/).
    """
    src = Path(source_path).expanduser()
    dst_folder = Path(dest_folder).expanduser()

    if not src.exists():
        return f"❌ Source not found: {src}"

    if not is_allowed_path(src):
        return f"❌ Access denied: {src}"

    try:
        dst_folder.mkdir(parents=True, exist_ok=True)
        dst = dst_folder / src.name
        shutil.copy2(src, dst)
        return f"✅ Uploaded (mock): {dst}"
    except Exception as e:
        return f"❌ Upload failed: {e}"


if __name__ == "__main__": 
    mcp.run()